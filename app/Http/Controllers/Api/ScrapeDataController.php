...
class ScrapeDataController extends Controller
{
    ...
    {
        $validated = $request->validate([ 'campaign_id' => 'required|integer', 'aggregatedData' => 'required|array' ]);
        $user = Auth::user();
        $reportsCreated = 0;
        $reportsSkipped = 0;
        ...
        foreach ($validated['aggregatedData'] as $dailyData) {
            try {
                ...
                $existingReport = CampaignReport::where('user_id', $user->id)
                    ->where('campaign_id', $validated['campaign_id'])
                    ->where('scrape_date', $dailyData['scrapeDate'])
                    ->first();
                ...
                // Jika tidak ada, lanjutkan proses penyimpanan
                DB::transaction(function () use ($user, $validated, $dailyData, &$reportsCreated) {
                    $data = $dailyData['data'];
                    ...
                    if (!empty($data['keywordPerformance'])) {
                        foreach ($data['keywordPerformance'] as $kw) {
                            $report->keywordPerformances()->create($this->flattenMetrics($kw, 'keyword'));
                        }
                    }

                    if (!empty($data['recommendationPerformance'])) {
                        foreach ($data['recommendationPerformance'] as $rec) {
                            $report->recommendationPerformances()->create($this->flattenMetrics($rec, 'recommendation'));
                        }
                    }
        ...
        return response()->json(['message' => 'Data berhasil diproses.', 'created' => $reportsCreated, 'skipped' => $reportsSkipped], 200);
    }
